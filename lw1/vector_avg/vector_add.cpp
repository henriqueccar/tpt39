...
//time code from https://stackoverflow.com/questions/10725758/opencl-gpu-execution-time-is-always-zero
...
int status;
	...
	for(unsigned j = 0; j < N; ++j) {
	      ...
	      ref_output = ref_output+input_a[j];
	    }
...
//    checkError(status, "Failed to set argument 2");
	...
	cout<<"GPU Elapsed time is :  "<< chrono::duration_cast<ns>(ndiff).count()<<" ns "<<endl;
// Verify results.
//bool pass = true;

//for(unsigned j = 0; j < N && pass; ++j) {
      if(fabsf(*output - ref_output) > 1.0e-5f) {
        printf("Failed verification @ index \n Output: %f\nReference: %f\n",*output, ref_output);
//        pass = false;
      }
	else 
